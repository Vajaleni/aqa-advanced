...
set author(newAuthor) {
    if(typeof newAuthor !=="string") throw new Error("Title must be a string");
...
set year(newYear) {
    if(typeof newYear !=="number") throw new Error("Year must be a positiv number");
    this._year = newYear;
}
...
return current.year < oldest.year ?
current : oldest});
...
class EBook extends Book {
    ...
    static convertToEBook(book, format) {
        ...
        return new
        EBook(book.title, book.author,book.year,format)
         }
